...
### Retry/backoff for the synchronous modules

**Why this was added.** `docs/PERFORMANCE_BASELINE.md` § 4 flagged this as a known gap: `module2_scoring`, `module3_resume`, and `module4_coverletter` (built on the sync `Anthropic` client) had no retry/backoff at all — a single transient network blip, timeout, or 5xx failed the whole item outright, unlike `module3b_resume_board`'s async calls, which already go through `with_retry`. It's a reliability gap independent of the prompt-caching or job-JSON-dump work — those change what's *in* the request; this changes what happens when the request itself fails.

**Design — `with_retry_sync`, alongside `with_retry`, not merged into it.** `modules/util.py` already separated the *decision* logic (`classify_error`'s four categories, `_retry_after_seconds`) from the *loop* (`with_retry`'s `await`/`asyncio.sleep`), so extending retry coverage to the sync modules meant reusing the same classification and message format, not re-deriving policy. `with_retry_sync(fn, retries=3, base_delay=2.0, max_delay=60.0)` is a synchronous twin: same four categories, same behavior per category, same log message format — just a plain callable instead of a coroutine function, and `time.sleep()` instead of `asyncio.sleep()`. It isn't merged into a single implementation, because doing so would mean either forcing the three sync call sites onto an event loop they don't otherwise need, or forcing `with_retry`'s existing, tested async callers (`module3b_resume_board`) through a sync-async bridge — a bigger, riskier change than this fix calls for. The classification behavior itself is identical (see `ARCHITECTURE.md` → Retry policy: three failure categories, three different responses, and `docs/PERFORMANCE_BASELINE.md` § 4 for why each category gets the treatment it does):

- **Transient** (network error, timeout, HTTP 500/502/503/504) — retried with exponential backoff.
- **Rate limited** (HTTP 429) — retried, honoring a `Retry-After` header when present, exponential backoff otherwise.
- **Quota exceeded** (account-level usage cap) — never retried, fails immediately with a clear notification.
- **Non-retryable** (auth failure, permission error, invalid request, not found) — never retried, no special notification.

**Wiring.** Each of the three modules now wraps its `tracked_create(...)` call in a nested `_call()` closure and passes it to `with_retry_sync(_call)`, mirroring the exact pattern `module3b_resume_board` already uses for `with_retry` (`async def _call(): ... return await with_retry(_call)`). Nothing about what's sent in the request changed — same `system`, same message blocks (including the `cache_control` breakpoints from the prompt-caching round), same `max_tokens`. Only the failure path changed.

**A near-miss caught while testing this.** The new integration tests initially called `score_job`/`tailor_resume`/`generate_cover_letter` for real with only `client.messages.create` mocked, which meant every retry attempt ran through the real (unmocked) `tracked_create` and `track_stage`, writing real entries into `data/llm_usage_log.jsonl` and `data/pipeline_stage_log.jsonl` — the same contamination class caught during the prompt-caching round. Fixed the same way: `tests/test_sync_module_retry.py` redirects both `util.LLM_USAGE_LOG_PATH` and `util.PIPELINE_STAGE_LOG_PATH` to tempfiles in `setUp`/`tearDown`, and patches `modules.util.time.sleep` so retry delays don't actually elapse.

**Remaining gap.** `module1_discovery` (HTTP scraping, not an Anthropic API call) and `module2b_board_review`'s fan-out reviewers/chair (already covered — they're async, already on `with_retry`) are unaffected by this change. Every LLM-calling module in the pipeline now has retry coverage; `module1_discovery`'s HTTP requests remain the one call path with no retry/backoff, since it's a different failure domain (LinkedIn/Greenhouse/Lever HTTP responses, not Anthropic API errors) that `classify_error` wasn't built to classify.

**Verification.** `tests/test_retry.py` gained four new test classes (`TestWithRetrySyncTransientAnd5xx`, `TestWithRetrySyncRateLimited`, `TestWithRetrySyncQuotaExceeded`, `TestWithRetrySyncNonRetryable`, 18 tests) mirroring the existing async coverage exactly, proving `with_retry_sync` behaves identically to `with_retry` per category. `tests/test_sync_module_retry.py` (12 tests, new) proves the wiring itself — not just the utility in isolation — by mocking only `client.messages.create` on each module's real client and letting `with_retry_sync`, `tracked_create`, and the module function all run for real: successful recovery after transient errors, exhaustion after `retries` attempts, 429 retried, and quota exhaustion failing on the first attempt, for all three modules. 159 tests total, all passing, zero real API calls.
